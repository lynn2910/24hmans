	...
	<div>
		<div class="container preload">
			<div class="creditcard">
				<div class="front">
					<div id="ccsingle" ref="ccsingle"></div>
...
export default {
	...
	mounted: function () {
		this.cardnumber_mask = new InputMask(this.$refs.cardnumber, {
			mask: [
				{
		...
		this.expirationdate_mask = new InputMask(this.$refs.expirationdate, {
			mask: Date,
			pattern: 'm{/}d',
			blocks: {
				m: {
		...
		this.securitycode_mask = new InputMask(this.$refs.securitycode, {
			mask: '0000',
		});

		// Création des listeners

		let cardnumber_mask = this.cardnumber_mask;

		let ccicon = this.$refs.ccicon;
		let ccsingle = this.$refs.ccsingle;
		let swapColor = this.swapColor;
		...
		let expirationdate_mask = this.expirationdate_mask;
		this.expirationdate_mask.on('accept', function () {
			if (expirationdate_mask.value.length === 0) {
				document.getElementById('svgexpire').innerHTML = '01/23';
			} else {
				console.log(expirationdate_mask.value);
				document.getElementById('svgexpire').innerHTML = expirationdate_mask.value;
			}
		});

		let securitycode_mask = this.securitycode_mask;
		this.securitycode_mask.on('accept', function () {
		...
		this.$refs.cardnumber.addEventListener('focus', function () {
			document.querySelector('.creditcard').classList.remove('flipped');
		});

		this.$refs.expirationdate.addEventListener('focus', function () {
			document.querySelector('.creditcard').classList.remove('flipped');
		});

		this.$refs.securitycode.addEventListener('focus', function () {
