<template>
	<div>
		<h1 class="text-2xl font-bold my-3">{{ $t('services.billetterie.date_title') }}</h1>
		<div>
			<div v-for="(forfait, index) in forfaits" :key="index">
                <span class="text-white text-lg font-bold cursor-pointer hover:underline"
											:class="{ 'underline': selectedForfaits.includes(forfait) }" @click="toggleForfait(forfait)">
                    {{ forfait.forfait_label }}
                </span>
			</div>
			<button class="mt-4 px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
							:disabled="selectedForfaits.length === 0" @click="emitSelectedForfaits">
				{{ $t('global.next') }}
			</button>
		</div>
	</div>
