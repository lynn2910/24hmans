		...
		<div class="p-6">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
				<div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 flex flex-col">
					...
					<ul v-if="latestTickets.length > 0" class="space-y-2">
						<li v-for="ticket in latestTickets" :key="ticket.id"
							...
							<div class="flex justify-between items-center">
								<div>
									<h3 class="text-md font-medium text-gray-800 dark:text-gray-100">{{ ticket.title }}</h3>
									<p class="text-sm text-gray-500 dark:text-gray-400">Date: {{ formatDate(ticket.date) }}</p>
								</div>
								<span :class="[
					...
					<div class="mt-auto">
						<a href="#" class="text-blue-600 dark:text-blue-400 hover:underline text-sm">Voir tous les billets</a>
					</div>
				</div>
				...
				<div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 flex flex-col">
					...
					<ul v-if="latestPurchases.length > 0" class="space-y-2">
						<li v-for="purchase in latestPurchases" :key="purchase.id"
							...
							<div class="flex justify-between items-center">
								<div>
									<h3 class="text-md font-medium text-gray-800 dark:text-gray-100">{{ purchase.item }}</h3>
									<p class="text-sm text-gray-500 dark:text-gray-400">Date: {{ formatDate(purchase.date) }}</p>
								</div>
								<span class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ purchase.amount }}€</span>
							</div>
						</li>
					</ul>
					...
					<div class="mt-auto">
						<router-link :to="{ name: 'client_panel_orders'}"
												 class="text-blue-600 dark:text-blue-400 hover:underline text-sm">Voir tous les achats
						</router-link>
					</div>
				</div>
				...
				<div class="bg-white dark:bg-gray-800 shadow-md rounded-lg p-4 flex flex-col">
					...
					<ul v-if="latestRaces.length > 0" class="space-y-2">
						<li v-for="race in latestRaces" :key="race.id"
							...
							<div class="flex justify-between items-center">
								<div>
									<h3 class="text-md font-medium text-gray-800 dark:text-gray-100">{{ race.event }}</h3>
									<p class="text-sm text-gray-500 dark:text-gray-400">Date: {{ formatDate(race.date) }}</p>
								</div>
					...
					<div class="mt-auto">
						<router-link :to="{name:'client_panel_karting'}"
												 class="text-blue-600 dark:text-blue-400 hover:underline text-sm">Voir toutes les courses
						</router-link>
					</div>
				</div>
			</div>
			...
			<div class="bg-white dark:bg-gray-800 mt-5 p-4 rounded-lg border-2 border-blue-600 dark:border-blue-400">
				<p>Le système de <strong>billets</strong>
					et de <strong>courses de kartings</strong>
					ne sont pas encore 100% fonctionnels. Les données de ces deux catégories sont fictives.
				</p>
			</div>
		</div>
	</UserDashboardTemplate>
